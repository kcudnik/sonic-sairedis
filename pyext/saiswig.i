%{
    ...
    sai_status_t sai_get_segmentroute_api(sai_segmentroute_api_t* out)
    {
        sai_segmentroute_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_SEGMENTROUTE, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
...
typedef struct _sai_segmentroute_api_t {
    sai_status_t create_segmentroute_sidlist(_Out_ sai_object_id_t *segmentroute_sidlist_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_segmentroute_sidlist(_In_ sai_object_id_t segmentroute_sidlist_id);
    sai_status_t set_segmentroute_sidlist_attribute(_In_ sai_object_id_t segmentroute_sidlist_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_segmentroute_sidlist_attribute(_In_ sai_object_id_t segmentroute_sidlist_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_segmentroute_sidlists(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_segmentroute_sidlists(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
